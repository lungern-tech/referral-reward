...
class ImageHandler extends Module {
  ...
  showImageDialog() {
    ...
    if (!this.imageDialog) {
      ...
      <div class="image-url-form"><input class="text-input" type="text" placeholder="https://example.com/img.png" /><span class="url-submit">${words[2]
        }</span></div>
  ...
  beforeUpload(file) {
    if (this.imageDialog) {
      ...
      const words = ['Input image url', "Insert"]
